import { SEASONS, PETS, PET_BY_ID, ITEMS as ITEM_CATALOG, AGE_CHOICES } from './data.js';
...
const DEFAULT_LOADOUT = ['field-axe','flint-spear','reed-bow','palisade','briar-trap','acorn-tower'];
...
function newPlayer(id, name, x, y, color, bot=false) { return { id,name,x,y,vx:0,vy:0,angle:0,color,hp:100,maxHp:100,xp:0,age:0,nextXp:80,resources:{wood:20,stone:10,food:3,gold:0},selected:0,loadout:[...DEFAULT_LOADOUT],unlocks:['field-axe'],choiceAge:null,cooldown:0,dead:false,kills:0,pets:[],bot,respawn:0,target:null }; }
...
function renderHotbar() {
  ...
  (me?.loadout||DEFAULT_LOADOUT).forEach((id,index)=>{ const item=ITEM_BY_ID.get(id); if(!item)return; const slot=document.createElement('div'); slot.className=`slot ${index===game.selected?'selected':''}`; slot.innerHTML=`<small>${index+1}</small><strong>${item.name}</strong><em>${itemCost(item)}</em>`; slot.onclick=()=>selectItem(index); bar.append(slot); });
}
...
function openAgeChoice(age){
  const options=(AGE_CHOICES[Math.min(15,age)]||[]).map(id=>ITEM_BY_ID.get(id)).filter(Boolean);if(!options.length)return;
  ...
  for(const item of options){const button=document.createElement('button');button.className='age-option';button.innerHTML=`<h3>${item.name}</h3><p>${item.category.toUpperCase()} · ${itemCost(item)||'STARTER'}</p><p>${describeItem(item)}</p>`;button.onclick=()=>chooseAgeItem(item.id);container.append(button)}
}
...
function applyChoice(player,id){const item=ITEM_BY_ID.get(id);if(!item)return;player.unlocks??=[];if(!player.unlocks.includes(id))player.unlocks.push(id);player.choiceAge=null;const slot=item.category==='building'?3:item.category==='ranged'?2:item.category==='melee'?1:0;player.loadout[slot]=id;renderHotbar();}
...
function gainXp(player,amount){ player.xp+=amount; while(player.xp>=player.nextXp){ player.xp-=player.nextXp; player.age++; player.nextXp=Math.round(player.nextXp*1.28); player.maxHp+=5; player.hp=player.maxHp; player.choiceAge=Math.min(15,player.age); if(player.bot){const choices=AGE_CHOICES[player.choiceAge]||[];applyChoice(player,choices[Math.floor(Math.random()*choices.length)]);}else if(player.id===game.id)toast(`Age ${player.age}: choose new knowledge`); } }
...
function performAction(player){
  const item=ITEM_BY_ID.get(player.loadout?.[player.selected])||ITEM_BY_ID.get('field-axe'); if(!player.unlocks?.includes(item.id)){ if(player.id===game.id)toast(`Choose ${item.name} at an age-up first`); player.cooldown=.3; return; }
