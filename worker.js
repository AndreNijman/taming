...
function newPlayer(id,name,x,y,color,bot=false){return{id,name,x,y,angle:0,color,hp:100,maxHp:100,xp:0,age:0,nextXp:40,resources:{wood:20,stone:10,food:25,gold:0},selected:0,loadout:[...STARTING_ITEMS],unlocks:[...STARTING_ITEMS],choiceAge:null,cooldown:0,dead:false,kills:0,pets:[],bot,respawn:0,target:null,input:{x:0,y:0,angle:0,attack:false,interact:false,selected:0},brain:0,goal:null};}
...
export class GameRoom {
  ...
  performAction(player){
    const selectedId=player.bot?player.loadout?.find(id=>ITEM_BY_ID.get(id)?.slot==='primary'):player.loadout?.[player.selected],item=ITEM_BY_ID.get(selectedId)||ITEM_BY_ID.get('hand');if(!player.unlocks?.includes(item.id)){player.cooldown=.3;return}if(item.category==='building'){this.placeStructure(player,item);return}
