...
function newPlayer(id,name,x,y,color,bot=false){return{id,name,x,y,angle:0,color,hp:100,maxHp:100,xp:0,age:0,nextXp:80,resources:{wood:20,stone:10,food:3,gold:0},selected:0,loadout:[...DEFAULT_LOADOUT],unlocks:['field-axe'],choiceAge:null,cooldown:0,dead:false,kills:0,pets:[],bot,respawn:0,target:null,input:{x:0,y:0,angle:0,attack:false,interact:false,selected:0},brain:0,goal:null};}
...
export class GameRoom {
  ...
  async onMessage(session,raw){
    ...
    if(message.t==='choose'&&this.started&&player.choiceAge!==null){const item=ITEM_BY_ID.get(String(message.id||'')),choices=AGE_CHOICES[Math.min(15,player.choiceAge)]||[];if(item&&choices.includes(item.id)){if(!player.unlocks.includes(item.id))player.unlocks.push(item.id);const slot=item.category==='building'?3:item.category==='ranged'?2:item.category==='melee'?1:0;player.loadout[slot]=item.id;player.choiceAge=null}return}
  ...
  gainXp(player,amount){player.xp+=amount;while(player.xp>=player.nextXp){player.xp-=player.nextXp;player.age++;player.nextXp=Math.round(player.nextXp*1.28);player.maxHp+=5;player.hp=player.maxHp;player.choiceAge=Math.min(15,player.age);if(player.bot){const choices=AGE_CHOICES[player.choiceAge]||[],item=ITEM_BY_ID.get(choices[Math.floor(Math.random()*choices.length)]);if(item){player.unlocks.push(item.id);const slot=item.category==='building'?3:item.category==='ranged'?2:item.category==='melee'?1:0;player.loadout[slot]=item.id}player.choiceAge=null}this.event(`${player.name} reached age ${player.age}`,player.id)}}
  performAction(player){
    const item=ITEM_BY_ID.get(player.loadout?.[player.selected])||ITEM_BY_ID.get('field-axe');if(!player.unlocks?.includes(item.id)){player.cooldown=.3;return}if(item.category==='building'){this.placeStructure(player,item);return}
